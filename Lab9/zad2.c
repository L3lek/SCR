    ...
    for (int i = data->start; i < data->end; i++) {
        for (int j = 0; j < data->num_passwords; j++) {
            ...
            if (strcmp(temp_hashed, data->hashed_passwords[j].hashed_password) == 0) {
                printf("Password for %s is %s\n", data->hashed_passwords[j].email, data->dictionary[j].password);
            }

			for(int k=0;k<100;k++){
            snprintf(temp, MAX_LEN, "%d%s", k, data->dictionary[i].password);
            bytes2md5(temp, strlen(temp), temp_hashed);
            if (strcmp(temp_hashed, data->hashed_passwords[j].hashed_password) == 0) {
			...
			snprintf(temp, MAX_LEN, "%s%d", data->dictionary[i].password, k);
            bytes2md5(temp, strlen(temp), temp_hashed);
            if (strcmp(temp_hashed, data->hashed_passwords[j].hashed_password) == 0) {
                printf("Password for %s is %s\n", data->hashed_passwords[j].email, temp);
            }
			...
			for (int l = 0; l < 100; l++) {
                    snprintf(temp, MAX_LEN, "%d%s%d", l, data->dictionary[i].password, k);
...
int main() {
    ...
    read_password_file("2.txt", hashed_passwords, &num_passwords, 1);
